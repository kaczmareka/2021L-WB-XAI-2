...
Według modelu ranger zdecydowanie najważniejszą zmienną w modelu ranger jest zmienna `plas` (importance 0.175), co oznacza, że po przelosowaniu jej wartości AUC modelu zmalało aż o 0.825. Pozostałe zmienne mają wskaźnik importance na poziomie ok. 0.1, wszystkie bardzo podobny.

W modelu Ada ponownie najważniejszą zmienną jest zmienna `plas` (importance 0.22), na drugim miejscu mamy `mass` (0.17). Najmniej istotną zmienną okazuje się ilość razy, które pacjentka była w ciąży.
