...
```{r}
a <- blood_m[blood_m$Recency==2 & blood$Time < 24 & blood_m$Intensity < 0.55 , 4] == 1
b <- blood_m[blood_m$Recency==2 & blood$Time < 24 & blood_m$Intensity > 0.55 & blood$Time < 24, 4] == 1
```

```{r}
cat(paste0(round(sum(a)/length(a), 2)*100, '% out of ', length(a), ' people with Time < 24 and Intensity < 0.55 did donate.\n'))
```

```{r}
cat(paste0(round(sum(b)/length(b), 2)*100, '% out of ', length(b), ' people with Time < 24 and Intensity > 0.55 did donate.\n'))
```
